...
func PatchIngress(apply *v1beta1.MyIngress) (*v1beta1.MyIngress, error) {
	...
	oldIngress, err := configs.Factory.Networking().V1().Ingresses().Lister().
	...
	_, err = configs.K8sClient.NetworkingV1().Ingresses(newMi.Namespace).
...
func CreateIngress(mi *v1beta1.MyIngress) error {
	...
	_, err = configs.K8sClient.NetworkingV1().Ingresses(mi.Namespace).
